package com.moyuan.cloud.security.filter;
...
public class JWTAuthorizationFilter extends BasicAuthenticationFilter {
    ...
    private UsernamePasswordAuthenticationToken getAuthentication(String authorization) {
        ...
        try {
            String cellphone = JwtTokenUtils.getCellphoneByToken(token);
            logger.info("checking cellphone:" + cellphone);
            //判断token是否是最后登录的token，不是则退出
            Jedis jedis = RedisPool.getJedis();
            jedis.select(0);
            String stoken = jedis.get(cellphone + "token");
            RedisPool.returnResource(jedis);
            if (!StringUtils.isEmpty(cellphone) && !authorization.equals(stoken)) {
                throw new IllegalArgumentException();
            }
            // 通过 token 获取用户具有的角色
            List<SimpleGrantedAuthority> userRolesByToken = JwtTokenUtils.getUserRolesByToken(token);
            if (!StringUtils.isEmpty(cellphone)) {
                return new UsernamePasswordAuthenticationToken(cellphone, null, userRolesByToken);
            }
